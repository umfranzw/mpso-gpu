    ...
    )
{
    ...
    for (k = 0; k < num_dims / m; k++)
    {
        ...
        if (particle_mask && dim_id < m / 4)
        {
            /* partial.xy += 100.0f * pow( pow(perm1_chunk.xy, (float2) 2.0f) - perm1_chunk.yz, (float2) 2.0f ) + pow(perm1_chunk.yz - 1, (float2) 2.0f); */
            /* partial.x += 100.0f * pow( pow(perm1_chunk.z, 2.0f) - perm1_chunk.z, 2.0f ) + pow(perm1_chunk.w - 1, 2.0f); */

            partial.xy += 100.0f * pow( pow(perm1_chunk.xy, (float2) 2.0f) - perm1_chunk.yz, (float2) 2.0f ) + pow(perm1_chunk.xy - 1, (float2) 2.0f);
