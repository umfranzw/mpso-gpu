    ...
    )
{
    ...
    uint num_vals = num_dims / 4; //number of pairs (there are this many values for each term)
    uint i;
    ...
    if (num_vals > 1 && num_vals % 2 && !dim_id && mask)
    {
        //dim_id is 0 here, so local_mem_offset is correct
        partial += vload2(0, scratch + local_mem_offset + (num_vals - 1) * 2);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
