    ...
    )
{
    ...
    for (k = 0; k < num_dims / (2 * m); k++)
    {
        ...
        if (particle_mask && !dim_id)
        {
            ...
            rot_result = partial.x - partial.y + 20 + M_E_F;
            accum += rot_result;
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    ...
    {
        float2 term1 = pow(extra_perm2_chunk, ((float2) 2));
        float2 term2 = native_cos(2 * M_PI_F * extra_perm2_chunk);

        partial += (float2) (term1.x + term1.y, term2.x + term2.y);
    }
