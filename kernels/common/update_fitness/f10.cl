    ...
    )
{
    ...
    for (k = 0; k < num_dims / (2 * m); k++)
    {
        ...
        if (particle_mask && dim_id < m / 4)
        {
            perm1_chunk = pow(perm1_chunk, (float4) 2) - 10 * native_cos(2 * M_PI_F * perm1_chunk) + 10;
    ...
    {
        perm2_chunk = pow(perm2_chunk, (float4) 2) - 10 * native_cos(2 * M_PI_F * perm2_chunk) + 10;
        ...
        vstore2(partial, 0, scratch + scratch_particle_offset + dim_id);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    {
        extra_perm2_chunk = pow(extra_perm2_chunk, (float2) 2) - 10 * native_cos(2 * M_PI_F * extra_perm2_chunk) + 10;
