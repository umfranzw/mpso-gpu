    ...
    )
{
    ...
    if ((num_vals / 2) > 1 && (num_vals / 2) % 2 && !dim_id && particle_mask && dim_mask)
    {
    ...
    if (particle_mask && !dim_id)
    {
        ...
        rot_result = partial.x - partial.y + 20.0f + M_E_F;
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (particle_mask && dim_id < (num_dims - m) / 4)
    {
        float4 term1 = pow(perm2_chunk, ((float4) 2));
        float4 term2 = native_cos(2 * M_PI_F * perm2_chunk);
    ...
    if (particle_mask && !dim_id)
    {
        ...
        scratch[local_id / threads_per_particle] = rot_result * 1000000.0f + (partial.x - partial.y + 20.0f + M_E_F);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
