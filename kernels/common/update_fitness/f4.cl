    ...
    )
{
    ...
    if (dim_id < (num_dims - m) / 4)// && particle_mask)
    {
    ...
    if (dim_id < (num_dims - m) / 4)// && particle_mask)
    {
        ...
        float4 exp_numer = ((float4) dim_id) * ((float4) 4) + ((float4) (0, 1, 2, 3));
        float4 exp_denom = (float4) (num_dims - m - 1);
        float4 exponent = (exp_numer / exp_denom) * 6.0f;
