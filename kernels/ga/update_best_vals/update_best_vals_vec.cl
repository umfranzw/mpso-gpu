...
__kernel void update_best_vals_vec_ga(
    ...
    )
{
    ...
    if (!local_id)// && group_id < num_swarms)
    {
        ...
        if (not_updated_ratio > unhealthy_ratio)
        {
            swarm_health[group_id]++;
        }

        //if it's not unhealthy for consecutive iterations, reset to zero
        else
        {
            swarm_health[group_id] = 0;
        }
        ...
        //printf("%u: %f, %u\n", group_id, not_updated_ratio, swarm_health[group_id]);
    }
}
