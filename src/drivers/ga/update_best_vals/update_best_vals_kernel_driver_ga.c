    ...
    )
{
    ...
    if (iter_num == 0)
    {
        /* calc_update_best_vals_sizes_ga( */
        /*     conf, */
        /*     &local_work_size, */
        /*     &global_work_size, */
        /*     &swarms_per_group, */
    ...
    #if LAUNCH_WARNINGS
    printf("Launching update_best_vals kernel.\n");
    printf("kernel function name: %s\n", get_kernel_name(&(kernels[UPDATE_BEST_VALS_VEC_KERNEL_GA2])));
    ...
    set_update_best_vals_vec_kernel_args_ga(
        conf,
        &(kernels[UPDATE_BEST_VALS_VEC_KERNEL_GA2]),
        bufs,
        swarms_per_group
        );
