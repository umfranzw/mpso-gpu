    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching update_pos_vel_ga kernel.\n");
    printf("global_work_size: %u\n", global_work_size);
    printf("local_work_size: NULL\n");
    #endif
    
    set_update_pos_vel_vec_kernel_args_ga(
        conf,
        &(kernels[UPDATE_POS_VEL_VEC_KERNEL_GA]),
        bufs,
        iter_index,
        rep
        );
    ...
    check_error(error, "Error launching update_pos_vel_ga kernel.");
