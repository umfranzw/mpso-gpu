    ...
    )
{
    ...
    check_error(error, "Error setting f18 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f18 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F18 benchmark kernel.\n");
    ...
    set_f18_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F18_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F18 benchmark kernel.");
