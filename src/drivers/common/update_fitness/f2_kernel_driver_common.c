    ...
    )
{
    ...
    check_error(error, "Error setting f2 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f2 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F2 benchmark kernel.\n");
    ...
    set_f2_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F2_KERNEL)]),
        bufs,
        particles_per_group
        );
    ...
    check_error(error, "Error launching F2 benchmark kernel.");
