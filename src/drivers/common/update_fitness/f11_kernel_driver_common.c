    ...
    )
{
    ...
    check_error(error, "Error setting f11 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f11 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F11 benchmark kernel.\n");
    ...
    set_f11_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F11_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F11 benchmark kernel.");
