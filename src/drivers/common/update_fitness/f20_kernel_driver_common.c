    ...
    )
{
    ...
    check_error(error, "Error setting f20 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f20 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F20 benchmark kernel.\n");
    ...
    set_f20_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F20_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F20 benchmark kernel.");
