    ...
    )
{
    ...
    check_error(error, "Error setting f24 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f24 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F24 benchmark kernel.\n");
    ...
    set_f24_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F24_KERNEL)]),
        bufs,
        particles_per_group
        );
    ...
    check_error(error, "Error launching F24 benchmark kernel.");
