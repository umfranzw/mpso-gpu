    ...
    )
{
    ...
    check_error(error, "Error setting f21 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f21 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F21 benchmark kernel.\n");
    ...
    set_f21_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F21_KERNEL)]),
        bufs,
        particles_per_group
        );
    ...
    check_error(error, "Error launching F21 benchmark kernel.");
