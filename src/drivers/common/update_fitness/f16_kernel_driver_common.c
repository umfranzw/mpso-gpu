    ...
    )
{
    ...
    check_error(error, "Error setting f16 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f16 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F16 benchmark kernel.\n");
    ...
    set_f16_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F16_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F16 benchmark kernel.");
