    ...
    )
{
    ...
    check_error(error, "Error setting f13 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f13 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //Permutations require d / 4 threads, function1 requires m / 4 threads, function2 requires (d / 2) / 4 threads.
        ...
        local_mem_per_particle = conf->num_dims;
    }

    #if LAUNCH_WARNINGS
    printf("Launching F13 benchmark kernel.\n");
    ...
    set_f13_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F13_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F13 benchmark kernel.");
