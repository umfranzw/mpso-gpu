    ...
    )
{
    ...
    check_error(error, "Error setting f12 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f12 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F12 benchmark kernel.\n");
    ...
    set_f12_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F12_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F12 benchmark kernel.");
