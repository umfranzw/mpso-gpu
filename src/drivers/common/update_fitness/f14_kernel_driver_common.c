    ...
    )
{
    ...
    check_error(error, "Error setting f14 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f14 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F14 benchmark kernel.\n");
    ...
    set_f14_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F14_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F14 benchmark kernel.");
