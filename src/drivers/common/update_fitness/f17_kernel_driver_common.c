    ...
    )
{
    ...
    check_error(error, "Error setting f17 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f17 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //Permutations require d / 4 threads,  function1 requires m / 4 threads. m / 4 <= d / 4
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F17 benchmark kernel.\n");
    ...
    set_f17_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F17_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F17 benchmark kernel.");
