    ...
    )
{
    ...
    check_error(error, "Error setting f10 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f10 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //Permutations require d / 4 threads, rotation requires at least m / 4 threads, function1 requires m / 4 threads, function2 requires (d / 2) / 4 threads.
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F10 benchmark kernel.\n");
    ...
    set_f10_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F10_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    /* clWaitForEvents(events->wait_list_lens[UPDATE_FITNESS_EVENT)], events->wait_lists[UPDATE_FITNESS_EVENT)]); */
    /* printf("Launching! %u\n", iter_index); */
    ...
    check_error(error, "Error launching F10 benchmark kernel.");
