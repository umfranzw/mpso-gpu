    ...
    )
{
    ...
    check_error(error, "Error setting f9 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f9 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F9 benchmark kernel.\n");
    ...
    set_f9_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F9_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F9 benchmark kernel.");
