    ...
    )
{
    ...
    check_error(error, "Error setting f8 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f8 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //figure out local memory requirements - no rotation in this one
        local_mem_per_particle = conf->num_dims;
    }

    #if LAUNCH_WARNINGS
    printf("Launching F8 benchmark kernel.\n");
    ...
    set_f8_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F8_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F8 benchmark kernel.");
