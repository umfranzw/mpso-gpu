    ...
    )
{
    ...
    check_error(error, "Error setting f15 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f15 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //Permutations require d / 4 threads, rotation requires at least m / 4 threads, function requires m / 4 threads
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F15 benchmark kernel.\n");
    ...
    set_f15_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F15_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F15 benchmark kernel.");
