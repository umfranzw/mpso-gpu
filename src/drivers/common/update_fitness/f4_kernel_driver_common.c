    ...
    )
{
    ...
    check_error(error, "Error setting f4 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f4 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    if (iter_index == 0)
    {
        ...
        //Permutations require d / 4 threads, rotation requires at least m / 4 threads, function1 requires m / 4 threads, function2 requires (d - m) / 4 threads. d/4 >= m/4, d/4 > (d-m)/4, so we launch d/4 threads.
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F4 benchmark kernel.\n");
    printf("global_work_size: %u\n", global_work_size);
    printf("local_work_size: %u\n", local_work_size);
    printf("particles_per_group: %u\n", particles_per_group);
    printf("local_mem per particle: %u\n", local_mem_per_particle);
    #endif

    set_f4_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F4_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F4 benchmark kernel.");
