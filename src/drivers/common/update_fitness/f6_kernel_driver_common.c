    ...
    )
{
    ...
    check_error(error, "Error setting f6 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f6 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F6 benchmark kernel.\n");
    ...
    set_f6_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F6_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F6 benchmark kernel.");
