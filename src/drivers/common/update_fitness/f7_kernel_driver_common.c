    ...
    )
{
    ...
    check_error(error, "Error setting f7 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f7 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F7 benchmark kernel.\n");
    ...
    set_f7_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F7_KERNEL)]),
        bufs,
        particles_per_group,
    ...
    check_error(error, "Error launching F7 benchmark kernel.");
