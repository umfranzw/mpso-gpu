    ...
    )
{
    ...
    check_error(error, "Error setting f23 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f23 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F23 benchmark kernel.\n");
    ...
    set_f23_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F23_KERNEL)]),
        bufs,
        particles_per_group
        );
    ...
    check_error(error, "Error launching F23 benchmark kernel.");
