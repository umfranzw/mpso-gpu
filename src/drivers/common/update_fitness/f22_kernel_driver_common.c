    ...
    )
{
    ...
    check_error(error, "Error setting f22 kernel arg %d\n", arg_index);
    arg_index++;
    ...
    check_error(error, "Error setting f22 kernel arg %u\n", arg_index);
    arg_index++;
    ...
    )
{
    ...
    #if LAUNCH_WARNINGS
    printf("Launching F22 benchmark kernel.\n");
    ...
    set_f22_kernel_args(
        conf,
        &(kernels[ALG_NAME_COMMON_CAPS(F22_KERNEL)]),
        bufs,
        particles_per_group
        );
    ...
    check_error(error, "Error launching F22 benchmark kernel.");
