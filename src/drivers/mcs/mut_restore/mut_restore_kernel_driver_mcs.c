    ...
    )
{
    ...
    if (iter_index == 1) //this kernel is launched for the first time on the second iteration
    {
        ...
        /* printf("prev particles_per_group: %u\n", particles_per_group); */
        /* printf("prev local_work_size: %u\n", local_work_size); */
        /* printf("prev global_work_size: %u\n", global_work_size); */
    ...
    set_mut_restore_kernel_args_mcs(
        conf,
        &(kernels[MUT_RESTORE_VEC_KERNEL_MCS]),
        bufs,
        particles_per_group
        );
